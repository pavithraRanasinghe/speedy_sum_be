...
async def text_summarize(request: TextSumRequest = Body(...)) -> Any:
    ...
    if(request.user != 0):
        await save_history(request.user, request.text)
...
@router.get("/history/{user_id}", response_description="Fetch History", status_code=status.HTTP_200_OK)
async def find_history(user_id: str):
    await user_collection.find_one({"id": user_id})

async def save_history(user_id: int, data: str):
